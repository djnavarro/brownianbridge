...
net_map <- read_csv(here::here("network_map.csv")) %>%
  arrange(xpos, ypos) %>% 
  mutate(
    id = paste0("dot",1:n()),
    frame = 3,
...
# add the network map as a frame
msg_map <- bind_rows(msg_map, net_map) %>%
  arrange(id, frame)
...
if(!anim) {
  ...
  pic <- base_pic +
    transition_states(
      states = frame,
      transition_length = 2,
      state_length = rep(c(2.5,1,1,1,2.5),3),
      wrap = TRUE
      )  +
    ease_aes('linear') +
    shadow_wake(.1)
  ...
  anim_save(here::here("network_animation.gif"))
}
